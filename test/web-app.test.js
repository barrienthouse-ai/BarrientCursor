...
describe('standalone web app', () => {
  ...
  it('opens the live workbook by ID and serves the current briefing', () => {
    const web = read('apps-script-web/PMR_Web.gs');
    assert.match(web, /function doGet\s*\(/);
    assert.match(web, /PMR_WEB_WORKBOOK_ID = '1UO5BzN7LmPovfpOWfbnfKwwtDVmUSM9y08mRSeNu46w'/);
    assert.match(web, /PMR_briefingHtml_\(\)/);
    assert.match(web, /Never add it to GEAUXCHEVROLETSALESLOG/);
    assert.doesNotMatch(web, /SpreadsheetApp\.getUi\(\)/);

    const sheets = read('apps-script/PMR_Sheets.gs');
    assert.match(sheets, /SpreadsheetApp\.openById\(id\)/);
    assert.match(sheets, /PMR_workbookId_\(\)/);

    const menu = read('apps-script/PMR_Menu.gs');
    assert.match(menu, /function PMR_briefingHtml_/);
    assert.match(menu, /createHtmlOutputFromFile\('PMR_App'\)/);
  ...
  it('protects existing parts, service, sales, and fleet tabs', () => {
    const config = read('apps-script/PMR_Config.gs');
    assert.match(config, /PARTS_TICKETS/);
    assert.match(config, /SMR_Dashboard/);
    assert.match(config, /SLM_Daily/);
    assert.match(config, /FLM_Working/);
    assert.match(config, /SVC_PARTS_REQUESTS/);
  });
});
